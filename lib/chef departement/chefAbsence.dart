...
import 'package:myapp/chef%20departement/chefFormulaire.dart';
...
class chefAbsence extends StatefulWidget {
  const chefAbsence({super.key});

  @override
  State<chefAbsence> createState() => _chefAbsenceState();
}

class _chefAbsenceState extends State<chefAbsence> {
  int _currentIndex = 1;
  ...
  void _navigateToFormulaire() {
    if ((_selectedDay != null) || (_rangeStart != null && _rangeEnd != null)) {
      ...
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => ChefFormulaire(
  ...
  DemandDataSource(
    ...
    if (demandes != null && users != null && currentUser != null) {
      for (var demande in demandes) {
        ...
        if (demande.status == 'Accepté' && demande.userId != currentUser.id) {
